...
void ForwardRenderer::render(World* world, float deltaTime) {
    ...
    for (auto entity : world->getEntities()) {
        ...
        if (auto light = entity->getComponent<LightComponent>(); light) {
            ...
            if (light->isFlashlight && playerComp &&
                !playerComp->flashlightOn) {
                continue;
            }
    ...
    for (const auto& command : opaqueCommands) {
        ...
        if (dynamic_cast<LitMaterial*>(command.material)) {
            ...
            command.material->shader->set("light_count",
                                          (int)lightCommands.size());
            command.material->shader->set("fog_enabled", fogEnabled);
            command.material->shader->set("M", M);
            command.material->shader->set("M_IT",
            ...
            for (size_t i = 0; i < lightCommands.size(); i++) {
                ...
                // Get light world position and direction from its entity
                // transform
                glm::mat4 lightMatrix =
                    light->getOwner()->getLocalToWorldMatrix();
                glm::vec3 lightPos =
                    glm::vec3(lightMatrix * glm::vec4(0, 0, 0, 1));
                glm::vec3 worldDirection = glm::normalize(
                    glm::vec3(lightMatrix * glm::vec4(light->direction, 0.0f)));

                command.material->shader->set(prefix + "type",
                                              (int)light->lightType);
                command.material->shader->set(prefix + "position", lightPos);
                command.material->shader->set(prefix + "direction",
                                              worldDirection);
                command.material->shader->set(prefix + "color",
                                              light->getEffectiveColor());
                command.material->shader->set(prefix + "attenuation",
                                              light->attenuation);
                command.material->shader->set(prefix + "inner_cone_angle",
        ...
        if (instancedRenderer->mesh && instancedRenderer->material &&
            ...
            if (instancedRenderer->mesh->getSubmeshCount() > 0) {
                ...
                for (size_t i = 0;
                    ...
                    submeshMaterial->shader->set("light_count",
                                                 (int)lightCommands.size());
                    ...
                    for (size_t li = 0; li < lightCommands.size(); li++) {
                        ...
                        std::string prefix =
                            "lights[" + std::to_string(li) + "].";
                        glm::mat4 lightMatrix =
                            light->getOwner()->getLocalToWorldMatrix();
                        glm::vec3 lightPos =
                            glm::vec3(lightMatrix * glm::vec4(0, 0, 0, 1));
                        glm::vec3 worldDirection = glm::normalize(glm::vec3(
                            lightMatrix * glm::vec4(light->direction, 0.0f)));
                        submeshMaterial->shader->set(prefix + "type",
                                                     (int)light->lightType);
                        submeshMaterial->shader->set(prefix + "position",
                                                     lightPos);
                        submeshMaterial->shader->set(prefix + "direction",
                                                     worldDirection);
                        submeshMaterial->shader->set(
                            prefix + "color", light->getEffectiveColor());
                        submeshMaterial->shader->set(prefix + "attenuation",
                                                     light->attenuation);
                        submeshMaterial->shader->set(
                            prefix + "inner_cone_angle",
                            light->inner_cone_angle);
                        submeshMaterial->shader->set(
                            prefix + "outer_cone_angle",
                            light->outer_cone_angle);
                    }
                ...
                // Set lighting uniforms for instanced lit materials
                instancedRenderer->material->shader->set("camera_position",
                                                         eye);
                instancedRenderer->material->shader->set(
                    "light_count", (int)lightCommands.size());
                instancedRenderer->material->shader->set("fog_enabled",
                                                         fogEnabled);
                ...
                for (size_t li = 0; li < lightCommands.size(); li++) {
                    ...
                    glm::vec3 worldDirection = glm::normalize(glm::vec3(
                        lightMatrix * glm::vec4(light->direction, 0.0f)));
                    instancedRenderer->material->shader->set(
                        prefix + "type", (int)light->lightType);
                    instancedRenderer->material->shader->set(
                        prefix + "position", lightPos);
                    instancedRenderer->material->shader->set(
                        prefix + "direction", worldDirection);
                    instancedRenderer->material->shader->set(
                        prefix + "color", light->getEffectiveColor());
                    instancedRenderer->material->shader->set(
                        prefix + "attenuation", light->attenuation);
                    instancedRenderer->material->shader->set(
                        prefix + "inner_cone_angle", light->inner_cone_angle);
                    instancedRenderer->material->shader->set(
                        prefix + "outer_cone_angle", light->outer_cone_angle);
                }
    ...
    if (postprocessMaterial) {
        ...
        postprocessMaterial->shader->set("maxHealth",
                                         postprocessUniforms.maxHealth);
...
void ForwardRenderer::setStaticParams(const float maxHealth,
                                      const float health) {
