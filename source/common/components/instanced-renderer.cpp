...
void InstancedRendererComponent::deserialize(const nlohmann::json& data) {
    ...
    if (data.contains("submeshMaterials") &&
        data["submeshMaterials"].is_object()) {
        for (auto& [submeshName, materialName] :
             data["submeshMaterials"].items()) {
            Material* mat =
                AssetLoader<Material>::get(materialName.get<std::string>());
            if (mat) {
    ...
    if (data.contains("Map")) {
        ...
        glm::vec2 worldSize = glm::vec2(data.value("worldWidth", 400.0f),
                                        data.value("worldHeight", 400.0f));
        ...
        if (data.contains("scaleMultiplier")) {
            ...
            scaleMultiplier = glm::vec3(s[0].get<float>(), s[1].get<float>(),
                                        s[2].get<float>());
        ...
        if (data.contains("rotationMultiplier")) {
            ...
            rotationMultiplier = glm::vec3(r[0].get<float>(), r[1].get<float>(),
                                           r[2].get<float>());
            hasRotationMultiplier = true;
        }
        if (data.contains("rotationRandomRange")) {
            auto r = data["rotationRandomRange"];
            rotationRandomRange = glm::vec3(
                r[0].get<float>(), r[1].get<float>(), r[2].get<float>());
        }
        ...
        if (data.contains("positionOffset")) {
            ...
            positionOffset = glm::vec3(p[0].get<float>(), p[1].get<float>(),
                                       p[2].get<float>());
            hasPositionOffset = true;
        }
        if (data.contains("positionRandomRange")) {
            auto p = data["positionRandomRange"];
            positionRandomRange = glm::vec3(
                p[0].get<float>(), p[1].get<float>(), p[2].get<float>());
        }

        // Load culling configuration (optional)
        maxRenderDistance = data.value("maxRenderDistance", 100.0f);
        cullingBoundingRadius = data.value("cullingBoundingRadius", 10.0f);
        ...
        for (const auto& inst : instances) {
            ...
            glm::vec3 finalRotation =
                glm::vec3(0.0f, inst.rotation, 0.0f) + inst.rotationRandom;
            ...
            // Apply scale
            glm::vec3 baseScale =
                glm::vec3(inst.scale * inst.widthScale, inst.scale,
                          inst.scale * inst.widthScale);
            ...
            transform = glm::rotate(transform, finalRotation.x,
                                    glm::vec3(1.0f, 0.0f, 0.0f));
            transform = glm::rotate(transform, finalRotation.y,
