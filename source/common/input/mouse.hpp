...
class Mouse {
   private:
    ...
    bool currentMouseButtons[GLFW_MOUSE_BUTTON_LAST + 1],
        previousMouseButtons[GLFW_MOUSE_BUTTON_LAST + 1];
   ...
   public:
    ...
    // update the mouse state (mainly moves current frame state to become the
    // previous frame state)
    void update() {
        if (!enabled) return;
        previousMousePosition = currentMousePosition;
        std::memcpy(previousMouseButtons, currentMouseButtons,
                    sizeof(previousMouseButtons));
        scrollOffset = glm::vec2();
    }
    ...
    // Was the mouse button unpressed in the previous frame but became pressed
    // in the current frame
    [[nodiscard]] bool justPressed(int button) const {
        return currentMouseButtons[button] && !previousMouseButtons[button];
    }

    // Was the mouse button pressed in the previous frame but became unpressed
    // in the current frame
    ...
    static void lockMouse(GLFWwindow* window) {
        ...
        // Center the cursor at the screen center
        int width, height;
